...
export const getDashboard = async (req, res) => {
  try {
    const recentBlogs = await Blog.find({}).sort({ createdAt: -1 }).limit(5); // Fixed sort field
    const blogs = await Blog.countDocuments();
    const comments = await Comment.countDocuments();
    const drafts = await Blog.countDocuments({ isPublished: false });

    // Fixed variable name: changed from getDashboard to dashboardData
    const dashboardData = { blogs, comments, drafts, recentBlogs };

    res.json({ success: true, dashboardData }); // Now returning the correct variable
  } catch (error) {
    res.json({ success: false, message: error.message });
  }
};
